...
@available(macOS 14.0, *)
final class WaterfallExamplesView: ChapterView {
                    ...
                    Waterfall(columns: 3, spacing: 8) {
                        for (index, imageData) in viewModel.imageDatas.enumerated() {
                            AsyncImage(url: imageData.url)
                                ...
                                .tappableView { tappedView in
                                    let detailView = ImageDetailView()
                                    detailView.imageData = imageData
                                    let controller = HostingViewController(rootView: detailView)
                                    controller.preferredContentSize = imageData.size
                                    // AppKit presents a sheet or a modal window;
                                    // there is no `present(_:animated:completion:)`,
                                    // and no peek-preview stage before a context
                                    // menu either, so upstream's `.previewProvider`
                                    // and `.previewBackgroundColor` are dropped
                                    // rather than stubbed.
                                    tappedView.parentViewController?.presentAsSheet(controller)
                                }
...
@available(macOS 14.0, *)
final class ImageDetailView: NSView {
