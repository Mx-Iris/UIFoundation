...
final class TappableViewExamplesView: ChapterView {
    @Observable
    ...
    override func updateProperties() {
        ...
        componentEngine.component = VStack(spacing: 40) {
                    ...
                    #CodeExampleNoInsets(
                        Text("Tap for Location", font: .body)
                            ...
                            .tappableView { view in
                                let location = view.clickGestureRecognizer.location(in: view)
            ...
            VStack(spacing: 10) {
                ...
                Text("Display context menus on long press using .contextMenuProvider modifier. Return an NSMenu with your desired actions.", font: .body).textColor(.secondaryLabel)
                #CodeExampleNoInsets(
                    Text("Long Press for Menu", font: .body)
                        ...
                        .contextMenuProvider { _ in
                            NSMenu(children: [
                                NSMenuItem(title: "Copy", systemImage: "doc.on.doc") {
                                    print("Copy selected")
                                },
                                NSMenuItem(title: "Share", systemImage: "square.and.arrow.up") {
                                    print("Share selected")
                                },
                                NSMenuItem(title: "Delete", systemImage: "trash") {
            ...
            VStack(spacing: 10) {
                ...
                #CodeExampleNoInsets(
                    HStack(spacing: 10, alignItems: .center) {
                        Image(systemName: "photo", withConfiguration: NSImage.SymbolConfiguration(pointSize: 30, weight: .regular))
                            .tintColor(.systemBlue)
                        VStack(spacing: 4) {
                            Text("Preview Image", font: .bodyBold)
                            Text("Right-click for the menu", font: .caption).textColor(.secondaryLabel)
                        }
                    }
                    .inset(16)
                    ...
                    // Upstream attaches a `.previewProvider` here, which UIKit
                    // shows as a peek above the context menu. AppKit has no
                    // such stage -- `NSMenu` is presented on its own -- so this
                    // TappableView offers no `previewProvider` at all rather
                    // than one that quietly does nothing.
                    .contextMenuProvider { _ in
                        NSMenu(children: [
                            NSMenuItem(title: "View Full Size", systemImage: "arrow.up.left.and.arrow.down.right") {
                ...
                VStack(spacing: 10) {
                    ...
                    #CodeExampleNoInsets(
                        Text("Fade on Touch", font: .body)
                            ...
                            .tappableViewConfig(TappableViewConfig(
                                onHighlightChanged: { view, isHighlighted in
                                    NSView.animate(withDuration: 0.15) {
                                        view.alphaValue = isHighlighted ? 0.5 : 1.0
                                    }
                                }
                            ))
                    )
            ...
            VStack(spacing: 10) {
                ...
                #CodeExampleNoInsets(
                    VStack(spacing: 12) {
                        HStack(spacing: 10, alignItems: .center) {
                            Image(systemName: "doc.text.fill", withConfiguration: NSImage.SymbolConfiguration(pointSize: 30, weight: .regular))
                                .tintColor(.systemBlue)
                            VStack(spacing: 4) {
                    ...
                    .contextMenuProvider { _ in
                        NSMenu(children: [
                            NSMenuItem(title: "Open", systemImage: "doc.text") {
                                print("Open")
                            },
                            NSMenuItem(title: "Share", systemImage: "square.and.arrow.up") {
                                print("Share")
                            },
                            NSMenuItem(title: "Rename", systemImage: "pencil") {
                                print("Rename")
                            },
                            NSMenuItem(title: "Move", systemImage: "folder") {
                                print("Move")
                            },
                            NSMenuItem(title: "Delete", systemImage: "trash") {
                                print("Delete")
                    ...
                    .tappableViewConfig(TappableViewConfig(
                        onHighlightChanged: { view, isHighlighted in
                            NSView.animate(withDuration: 0.2) {
                                ...
                                view.layer?.backgroundColor = (isHighlighted ? NSColor.systemGray5 : NSColor.systemGray6).cgColor
                            }
                        }
                    ))
                )
                    ...
                    Flow(spacing: 10) {
                        for (icon, label) in [
                            ...
                            VStack(spacing: 8, justifyContent: .center, alignItems: .center) {
                                Image(systemName: icon, withConfiguration: NSImage.SymbolConfiguration(pointSize: 30, weight: .regular))
                                    .tintColor(.systemBlue)
                            ...
                            .tappableViewConfig(TappableViewConfig(
                                onHighlightChanged: { view, isHighlighted in
                                    NSView.animate(withDuration: 0.15) {
                                        ...
                                        view.alphaValue = isHighlighted ? 0.7 : 1.0
                                    }
                                }
                            ))
                        }
