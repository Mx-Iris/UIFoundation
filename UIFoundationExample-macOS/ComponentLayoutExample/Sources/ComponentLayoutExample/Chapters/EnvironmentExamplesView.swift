...
final class EnvironmentExamplesView: ChapterView {
    @Observable
                    ...
                    VStack(spacing: 10) {
                        ...
                        #CodeExample(
                            VStack(spacing: 16) {
                                ...
                                VStack(spacing: 8) {
                                    Text("Breaking: UIFoundationComponent 2.0 Released", font: .title)
    ...
    func layout(_ constraint: Constraint) -> some RenderNode {
        if let user = user {
            ...
            return VStack(spacing: 12, alignItems: .center) {
                Image(systemName: "person.circle", withConfiguration: NSImage.SymbolConfiguration(pointSize: 40, weight: .regular))
