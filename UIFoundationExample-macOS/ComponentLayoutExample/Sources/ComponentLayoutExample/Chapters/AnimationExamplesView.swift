...
import AppKit
// `NSView.transform` is an AppKitPlus category property.
import AppKitPlus
import UIFoundationComponent

@available(macOS 14.0, *)
final class AnimationExamplesView: ChapterView {
    ...
    override func updateProperties() {
        ...
        componentEngine.component = VStack(spacing: 40) {
            ...
            VStack(spacing: 10) {
                ...
                Text("UIFoundationComponent allows you to configure an Animator to animate view transitions. Whenever a view is inserted, deleted, or its frame is updated, the animator performs the corresponding animation.", font: .body).textColor(.secondaryLabel)
                VStack(spacing: 8) {
                            ...
                            for item in viewModel.customAnimItems {
                                Text(item, font: .body)
                                    ...
                                    .animateInsert { hostingView, view, frame in
                                        ...
                                        NSView.animate(withDuration: 0.5, delay: 0, usingSpringWithDamping: 0.6, initialSpringVelocity: 0) {
                                            view.alphaValue = 1
            ...
            VStack(spacing: 10) {
                ...
                #CodeExampleNoInsets(
                    ...
                    .tappableView {
                        let tapLocation = $0.clickGestureRecognizer.location(in: $0)
            ...
            VStack(spacing: 10) {
                ...
                VStack(spacing: 10) {
                    VStack(spacing: 6) {
                        ...
                        Text("If a component doesn't have a stable id, UIFoundationComponent cannot track it across updates, leading to unexpected animation behavior. Always assign unique ids to animatable components.", font: .body).textColor(.secondaryLabel)
                        
                        Text("❌ Without id: Wrong item animates", font: .caption).textColor(.systemRed)
                        Text("Tap any item to delete it. Notice how the animation happens on the wrong item because UIFoundationComponent can't track which item was removed.", font: .caption).textColor(.secondaryLabel)
                        #CodeExampleNoInsets(
                            VStack(spacing: 8, alignItems: .stretch) {
                                for (i, item) in viewModel.sharedItems.enumerated() {
                                    Text("Item \(item.index): Tap to Delete", font: .body)
                                        ...
                                        .cornerRadius(8)
                                        // ❌ No .id() modifier - UIFoundationComponent can't track items
                                }
                                Text("Add Item")
                    ...
                    VStack(spacing: 4) {
                        ...
                        ChapterLink(
                            ...
                            chapterIdentifier: "view-hierarchy",
            ...
            VStack(spacing: 10) {
                ...
                VStack(spacing: 8) {
                    ...
                    VStack(spacing: 4) {
                        ...
                        Text("Spring animations feel more natural. Use NSView.animate with usingSpringWithDamping for bouncy, organic motion.", font: .body).textColor(.secondaryLabel)
                    }
                    ...
                    VStack(spacing: 4) {
                        ...
                        Text("In delete animations, always call the completion block when done. UIFoundationComponent needs this to clean up views properly.", font: .body).textColor(.secondaryLabel)
                    }
        ...
        }.inset(24).ignoreHeightConstraint().scrollView()
            .with(\.componentEngine.visibleFrameInsets, NSEdgeInsets(top: -400, left: 0, bottom: -400, right: 0))
...
struct CustomFadeAnimator: Animator {
    ...
    func insert(hostingView: NSView, view: NSView, frame: CGRect) {
        view.bounds.size = frame.size
        view.center = CGPoint(x: frame.midX, y: frame.midY)
    ...
    func delete(hostingView: NSView, view: NSView,
    ...
    func update(hostingView: NSView, view: NSView, frame: CGRect) {
        NSView.animate(withDuration: duration) {
            view.frame = frame
        }
    }
...
@available(macOS 14.0, *)
@GenerateCode
final class AnimatedListExample: NSView {
    ...
    override func updateProperties() {
        ...
        componentEngine.component = VStack(alignItems: .stretch) {
            ...
            .scrollView()
            .with(\.componentEngine.visibleFrameInsets, NSEdgeInsets(top: -200, left: 0, bottom: -200, right: 0))
            .fill()
            .flex()
        }
        .view()
