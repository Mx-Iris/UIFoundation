...
public typealias UpdateComponent<Content: Component> = ModifierComponent<Content, UpdateRenderNode<Content.ResultRenderNode>>

/// A component produced by the ``Component/with(_:_:)`` modifier
public typealias KeyPathUpdateComponent<Content: Component, Value> = ModifierComponent<Content, KeyPathUpdateRenderNode<Value, Content.ResultRenderNode>>
...
extension Component {
    ...
    public subscript<Value>(dynamicMember keyPath: ReferenceWritableKeyPath<ResultRenderNode.View, Value>) -> (Value) -> KeyPathUpdateComponent<Self, Value> {
        { value in
            with(keyPath, value)
        }
    }
    ...
    public func with<Value>(_ keyPath: ReferenceWritableKeyPath<ResultRenderNode.View, Value>, _ value: Value) -> KeyPathUpdateComponent<Self, Value> {
    ...
    public func update(_ update: @escaping (ResultRenderNode.View) -> Void) -> UpdateComponent<Self> {
        ModifierComponent(content: self) {
            $0.update(update)
        }
    }
    ...
    public func roundedCorner() -> UpdateComponent<Self> {
        ModifierComponent(content: self) { node in
            node.update { view in
                view.backingLayer.cornerRadius = min(node.size.width, node.size.height) / 2
            }
        }
    }
}
