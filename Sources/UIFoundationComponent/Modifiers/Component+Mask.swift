...
public struct MaskComponent<Content: Component, Mask: Component>: Component {
    ...
    public func layout(_ constraint: Constraint) -> some RenderNode {
        ...
        return content
            .update { view in
                let maskingView: ComponentView
                if let existingMask = view.maskingView as? ComponentView {
                    maskingView = existingMask
                } else {
                    maskingView = ComponentView()
                    view.maskingView = maskingView
                }
                maskingView.frame = CGRect(origin: .zero, size: maskRenderNode.size)
                maskingView.componentEngine.reloadWithExisting(component: mask, renderNode: maskRenderNode)
            }
    ...
    public func layout(_ constraint: Constraint) -> some RenderNode {
        ...
        return contentRenderNode
            .update { view in
                ...
                maskingView.frame = CGRect(origin: .zero, size: contentRenderNode.size)
                maskingView.componentEngine.component = mask.size(contentRenderNode.size)
            }
    }
}
