...
open class ReuseManager: NSObject {
    ...
    public func enqueue<ReusedView: NSUIView>(identifier: String = NSStringFromClass(ReusedView.self), view: ReusedView) {
        view.componentReuseContext.reuseIdentifier = nil
        view.componentReuseContext.reuseManager = nil
    ...
    public func dequeue<ReusedView: NSUIView>(
        identifier: String = NSStringFromClass(ReusedView.self),
        _ defaultView: @autoclosure () -> ReusedView
    ) -> ReusedView {
        let queuedView = reusableViews[identifier]?.popLast() as? ReusedView
        let view = queuedView ?? defaultView()
        view.backingLayer.removeAllAnimations()
        (view as? ReuseableView)?.prepareForReuse()
        if !removeFromSuperviewWhenReuse {
            view.isHidden = false
        }
        view.componentReuseContext.reuseManager = self
        view.componentReuseContext.reuseIdentifier = identifier
        return view
    }
