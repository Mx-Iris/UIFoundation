...
@available(macOS 11.0, *)
private let _deviceSymbolNameMap: [String: String] = [
    // Mac
...
@available(macOS 11.0, *)
private let _deviceModelCodeTagClass = UTTagClass(rawValue: "com.apple.device-model-code")

/// Resolves the SF Symbol name for a UTType by walking its supertype chain.
@available(macOS 11.0, *)
private func _resolveSymbolName(for type: UTType) -> String? {
...
extension FrameworkToolbox where Base: NSWorkspace {
    ...
    public func deviceIcon(forModelIdentifier modelIdentifier: String) -> NSImage {
        if let type = UTType(tag: modelIdentifier, tagClass: _deviceModelCodeTagClass, conformingTo: nil) {
    ...
    public func deviceSymbolName(forModelIdentifier modelIdentifier: String) -> String? {
        guard let type = UTType(tag: modelIdentifier, tagClass: _deviceModelCodeTagClass, conformingTo: nil) else {
            return nil
        }
        return _resolveSymbolName(for: type)
    }
