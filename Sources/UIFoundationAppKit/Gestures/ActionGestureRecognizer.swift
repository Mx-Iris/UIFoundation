...
open class ActionGestureRecognizer: NSGestureRecognizer {
    ...
    open class GestureRecognizerEventObservation {
        ...
        open var isObserving: Bool {
            ...
            set {
                ...
                if newValue {
                    ...
                    if let index = gestureRecognizer?.actionObservations[type, default: []].firstIndex(where: { $0.id == id }) {
                        gestureRecognizer?.actionObservations[type, default: []].remove(at: index)
                    }
                }
            }
        }
        ...
        init(type: NSEvent.EventType, gestureRecognizer: ActionGestureRecognizer, handler: @escaping (NSEvent) -> Void) {
            self.type = type
    ...
    open func observe(_ type: NSEvent.EventType, handler: @escaping (_ event: NSEvent) -> Void) -> GestureRecognizerEventObservation {
        .init(type: type, gestureRecognizer: self, handler: handler)
    }

    var actionObservations: [NSEvent.EventType: [(id: UUID, handler: (NSEvent) -> Void)]] = [:]
