    ...
    public func configurate(using configuration: ImageConfiguration) {
        ...
        if var imageSize = image?.size {
            ...
            case let (.some(maxWidth), .some(maxHeight)):
                imageSize = imageSize.scaled(toFit: CGSize(width: maxWidth, height: maxHeight))
