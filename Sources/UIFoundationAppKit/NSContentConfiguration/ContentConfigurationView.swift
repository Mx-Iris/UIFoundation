...
open class ContentConfigurationView: NSView {
    ...
    open var configurationState: NSViewConfigurationState {
        NSViewConfigurationState(isSelected: isSelected, isEnabled: isEnabled, isHovered: isHovered, isEditing: isEditing, isEmphasized: isEmphasized, customStates: [:])
    ...
    func sharedInit() {
        ...
        trackingArea?.update()
        windowHandlers.isKey = { [weak self] _ in
