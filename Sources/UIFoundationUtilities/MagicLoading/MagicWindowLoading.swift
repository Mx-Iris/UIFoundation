...
extension NSWindowController {
    fileprivate func loadWindowIfNeeded() {
        assert(Thread.isMainThread)
        if isWindowLoaded == false {
            _ = window // Invokes loadWindow(), but only ever once.
        }
    }

    @propertyWrapper
    public struct MagicWindowLoading<WrappedValue> {
