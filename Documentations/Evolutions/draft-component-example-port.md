...
| 2026-09-22 | **7. 滚动掉帧的真凶:`CodeTextView` 每个实例自带一个 JavaScript 虚拟机** | 用户报告滚动卡顿,并指出上游 Example 也有。实测(release):`Highlightr()` 构造 **25–33 ms**,而一次高亮只要 0.48 ms —— 高亮本身只占 1.5%。`CodeTextView` 写的是 `private let highlighter = Highlightr()`,而 UIComponent 5.0 起**视图复用是 opt-in 的**(要显式 `.reuseKey()`),章节里的代码块没加,于是每个代码块滚进视口都新建一个视图、也就是新建一个 JSContext。60fps 一帧是 16.6 ms,**一个代码块 = 丢两帧**。这解释了上游为什么同样卡:它是同一个形状,与 AppKit 移植无关 |
| 2026-09-22 | 先排除了布局引擎 | `VerticalRenderNode` / `HorizontalRenderNode` 用二分查找定位可见区间(只有 `SlowRenderNode` 是线性扫描);合成一棵章节形状的树实测,可见元素收集 **0.04 ms/tick 且不随内容增长**(10 / 40 / 120 个 section 都是 0.04),整个 tick 在 120 section、35000pt 内容下是 0.62 ms。比一个代码块便宜五十倍 —— 引擎不是瓶颈 |
| 2026-09-22 | **换 tree-sitter,而不是只把 Highlightr 共享掉** | 两件事一起做。共享+记忆化能把 33 ms 压到 0.00 ms,但 highlight.js 的 Swift 语法定义是**正则**堆的,质量有硬伤:实测它把 `.inset(` 切成 `.in` + `set`,并把 `set` 当成属性访问器**关键字**染成紫色;同一段里 `.backgroundColor` / `.systemBlue` / `.flexGrow` / `.size` / `.fill` 全是无色纯文本。tree-sitter 是真解析器,同一段给出 `function.call inset`、`variable.member`、`variable.parameter`,全部正确 |
| 2026-09-22 | 性能对照(release,每个代码块) | 构造引擎 Highlightr **25–33 ms** → tree-sitter **0.000 ms**;一次高亮 0.48 ms → **0.086 ms**;重复访问同一段 0.48 ms → **0.00 ms**(记忆化命中)。新增的唯一一次性开销是编译 `highlights.scm` 的 **78.6 ms**,放在 `HomeView.init` 里用后台队列预热,远早于第一个代码块 |
| 2026-09-22 | 语法包必须依赖 `with-generated-files` 分支 | `alex-pinkus/tree-sitter-swift` 的 tag 只带 grammar 源码,期望使用者自己跑 `tree-sitter generate`,SwiftPM 消费者做不到。只有 `with-generated-files` 分支带 `src/parser.c`、`Package.swift` 和 `queries/`。`ChimeHQ/SwiftTreeSitter` 已迁到官方组织 `tree-sitter/swift-tree-sitter` |
| 2026-09-22 | query 的 capture 必须**先到先得** | tree-sitter 的约定是「第一个匹配的 pattern 胜出」。`highlights.scm` 末尾有一条兜底的 `@variable`,如果按出现顺序无条件覆盖,它会把前面所有 `@function.call` 全部刷掉,每个调用点都变成普通标识符的颜色。实现里用一个 `claimed` 位图,只给尚未认领的字符上色。另外 `@spell`(给拼写检查用的注释正文)必须显式跳过,否则它会先认领掉注释的范围,让 `@comment` 够不着 |
| 2026-09-22 | 调色板沿用 One Dark / One Light | Highlightr 白送 271 套主题,tree-sitter 只给 capture 名,主题得自己写 —— 这是换的代价。深色那几个值是直接从换之前的输出里读出来的(`#C678DD` 关键字 / `#98C379` 字符串 / `#D19A66` 数字 / `#ABB2BF` 正文),所以代码块的观感没有跟着变。capture 名按**最长前缀**回退解析,`keyword.conditional` 之类十四个变体不用逐个登记 |
| 2026-09-22 | `variable` / 括号 / 运算符留白 | `@variable` 是 `highlights.scm` 的兜底 capture,给它上色等于给每个参数标签和局部变量上色。括号同理 —— 一个基本由括号组成的布局 DSL,把括号染色只剩噪音。代价是 `.systemBlue` / `.fill` 这类隐式成员目前是纯文本色(语法树只给它们 `@variable`) |
