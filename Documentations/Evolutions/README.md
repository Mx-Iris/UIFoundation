...
| [0014](0014-running-application-merge.md) | RunningApplication：把 RunningApplicationKit 整体并入本库 | Implemented | 把作者自己的独立仓库 `Mx-Iris/RunningApplicationKit`（运行中的应用与 BSD 进程的值类型模型、两个观察者、带搜索与排序的选择器）整体搬进本库，落在新 trait `RunningApplication`（默认关闭）下的**独立 target** `UIFoundationRunningApplication`，仅 macOS 11+ —— 不进伞包，因为它的平台下限高于伞包的 10.15。分两阶段落地：先原样搬运（与原库逐行可比，0 处预期外差异），再接入本库基座重写（`.box.makeView`、`TableCellView`、`makeConstraints`、`HStackView`、`XiblessViewController`）。**重写前先把布局测试扩到覆盖每个 cell 的关键几何并确认其在旧实现上全绿** —— 原库有四个布局 bug 编译干净、测试全过却只在截图里现形。顺带删除原库标注「下个 minor 移除」的弃用别名。**本次只做本库这一侧**：原仓库原地保留，四个下游（RuntimeViewer / HopperMCP / MachInjector / LookInside）的迁移与原仓库归档另开一轮。配套使用指南见 [`RunningApplication.md`](../RunningApplication.md)。 |
| [0015](0015-simulator-platform-detection.md) | 进程平台识别与模拟器标记 | Implemented | **随 0014 从 RunningApplicationKit 并入，原编号 0001，正文保持原貌。** 读进程可执行文件的 Mach-O `LC_BUILD_VERSION` 判定它被编译成给哪个平台跑，据此在进程列表里标出模拟器 guest 进程 —— 架构做不到这件事，因为 Apple Silicon 上模拟器里的进程跑的是原生 arm64，与宿主进程完全一致。配套实现说明见 [`Internal/PlatformDetection.md`](../Internal/PlatformDetection.md)。 |
| [0016](0016-picker-presentation-styles.md) | 选择器呈现样式（表格与列表） | Implemented | **随 0014 从 RunningApplicationKit 并入，原编号 0002，正文保持原貌。** 给两个标签页各加一个 `style`，在表格之外新增列表样式（图标 + 标题 + 行内徽章 + 副标题）；两种样式对「不值一提的值」处理相反 —— 列表省略、表格用暗色打印，因为一列空白读起来像坏了。`allowsColumns` 改名 `allowsFields`（该弃用别名已由 0014 删除）。配套实现说明见 [`Internal/PresentationStyles.md`](../Internal/PresentationStyles.md)。 |
