...
| 2026-08-10 | 新增 `NavigationPageBackdrop`，默认 `.automatic` | macOS 26 上页面为了透出玻璃必须设 `.clear`，而转场的前提是「新页压住旧页」—— 两个透明页并排滑动会同时显示两套内容。改为在**行进中那一页**底下垫一层背景视图，结束移除。**没有采用「改页面自己的 `layer.backgroundColor`、结束再还原」**：本仓库的 `LayerBackedView` 就在 `updateLayer()` 里写这个属性，转场中一次重绘就会覆盖它，还原时还会写回过期值；兄弟视图没有需要还原的状态。 |
| 2026-08-10 | push 与 pop 用两种方式落地同一条规则 | push 在新页**底下插一层**（新页从屏外进场，垫层跟着一起进来，不会凭空出现）；pop 则把背景色**直接写在离场页自己身上**、结束还原并 `needsDisplay = true`（离场页一开始就铺满容器，在它底下插一层会让整个容器在按下返回的瞬间闪一下）。还原后强制重绘是必需的 —— 基于 `LayerBackedView` 的页面会在 `updateLayer()` 里重写背景色。 |
| 2026-08-10 | 背景只给「跑完整位移的那一页」：push 是新页，pop 是**离场页** | 那是视线跟随的那一页，也是必须看起来实心、才不会透出背后那页的那一页。另一页只走视差位移，不加背景，容器从它透出来。中途试过两种都被否：只按角色垫「要去的那一页」，以及两页都垫（后者让几乎不动的那页也变实心，整个容器在转场期间翻成一块平面）。 | 只加一页会让容器一半实心一半透光，中间有一条随页面移动的接缝；弹出时尤其明显 —— 离场页是实心板，它背后被揭开的返回页却还透着玻璃。两页都加，整个容器在转场期间是一块统一的表面，首尾各翻一次。做法与 RuntimeViewer 的 `SidebarNavigationController` 手写版一致（它在 `UXNavigationController` 的 `willShow` 里同时给 from / to 两个控制器设背景色，结束再各自还原）。 |
